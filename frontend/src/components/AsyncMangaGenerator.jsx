                      ...
                      <div key={panelNumber} style={{ marginBottom: '20px' }}>
                        ...
                        <div style={{
                          ...
                          {panelGroups[panelNumber]
                            .sort((a, b) => {
                              // First sort by is_regenerated (original first)
                              if (a.is_regenerated !== b.is_regenerated) {
                                return a.is_regenerated - b.is_regenerated;
                              }
                              // Then sort regenerated panels by version (ascending)
                              if (a.is_regenerated && b.is_regenerated) {
                                return a.version - b.version;
                              }
                              // For original panels, maintain original order
                              return 0;
                            })
                            ...
                            <ul style={{ margin: '4px 0 0 0', paddingLeft: '16px' }}>
                              {panelGroups[panelNumber]
                                ...
                                <li key={panel.id} style={{ marginBottom: '2px' }}>
                                  Version {panel.version}: "{panel.regeneration_request}"
                                </li>
                              ))}
                            </ul>
                          </div>
              ...
              {taskDetails.panels && taskDetails.panels.length > 0 && (
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <div style={{
                    ...
                    {(() => {
                      // Get the best panel for each panel number (prioritize latest regenerated by version)
                      const panelGroups = {};
                      taskDetails.panels.forEach(panel => {
                        const panelNumber = panel.panel_number;
                        if (!panelGroups[panelNumber]) {
                          panelGroups[panelNumber] = panel;
                        } else {
                          const currentPanel = panelGroups[panelNumber];
                          // Prioritize regenerated panels, and among regenerated panels, choose the highest version
                          if (panel.is_regenerated && !currentPanel.is_regenerated) {
                            panelGroups[panelNumber] = panel;
                          } else if (panel.is_regenerated && currentPanel.is_regenerated && panel.version > currentPanel.version) {
                            panelGroups[panelNumber] = panel;
                          }
                        }
                      });
